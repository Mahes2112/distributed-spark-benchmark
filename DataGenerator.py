...
with open("ecommerce_10M_55cols.csv", "w", newline="", encoding="utf-8") as file:
    ...
    writer.writerow([
        "transaction_id","user_id","first_name","last_name","age","gender",
        "phone","email","city","state","pincode","registration_date",
        "order_id","order_date","order_status","order_channel",
        "product_id","product_name","category","quantity","price_per_unit","total_price",
        "discount","tax","final_price",
        "payment_method","payment_status","transaction_ref",
        "bank_name","card_type",
        "shipment_id","warehouse_id","courier_partner",
        "shipment_date","delivery_date","delivery_status",
        "return_requested","return_reason",
        "device_type","browser","ip_address","session_duration",
        "traffic_source","campaign_name","coupon_code",
        "loyalty_points","customer_rating",
        "is_prime_user","fraud_score","is_high_risk",
        "latitude","longitude","currency","emi_option",
        "gift_wrap","referral_code"
    ])
    ...
    for i in range(1, NUM_RECORDS + 1):
        ...
        writer.writerow([
            i,user_id,first,last,age,gender,
            phone,email,city,state,pincode,reg_date.strftime("%Y-%m-%d"),
            order_id,order_date.strftime("%Y-%m-%d"),order_status,order_channel,
            product_id,product_name,category,quantity,price,total,
            discount,tax,final,
            payment_method,payment_status,transaction_ref,
            bank_name,card_type,
            shipment_id,warehouse_id,courier,
            shipment_date.strftime("%Y-%m-%d"),
            delivery_date.strftime("%Y-%m-%d"),delivery_status,
            return_requested,return_reason,
            device_type,browser,ip_address,session_duration,
            traffic,campaign,coupon,
            loyalty,rating,
            is_prime,fraud_score,is_high_risk,
            latitude,longitude,currency,emi_option,
            gift_wrap,referral
        ])
